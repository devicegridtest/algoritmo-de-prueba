...
<div style="display: flex; justify-content: center; align-items: center; gap: 15px; margin-bottom: 20px; padding: 10px; background: rgba(0, 255, 255, 0.05); border-radius: 10px; box-shadow: 0 0 15px rgba(0, 255, 255, 0.5);">
    ...
    <h1 style="margin: 0; font-size: 2rem; text-shadow: 0 0 15px rgba(0, 255, 255, 0.8); color: #0ff; letter-spacing: 1px;">📈 Cripto Tracker — Seguimiento en Tiempo Real + Predicción IA</h1>
</div>
""", unsafe_allow_html=True)

# --- VALIDACIÓN DE PERIODO/INTERVALO ---
valid_intervals = {
    "1d": ["1m", "5m", "15m", "30m", "1h"],
    "5d": ["1m", "5m", "15m", "30m", "1h"],
...
with st.sidebar:
    ...
    interval = st.selectbox("Intervalo", allowed_intervals, index=allowed_intervals.index(default_interval))
    enable_alerts = st.checkbox("🔔 Alertas RSI", value=True)
    enable_news = st.checkbox("📰 Noticias", value=False)
    st.markdown("---")
    st.caption("🔄 Auto-refresco cada 60s")
    ...
    try:
        ...
        if data.empty:
            if st.session_state.data is not None and len(st.session_state.data) > 0:
                ...
                st.error("⚠️ Datos insuficientes tras calcular indicadores.")
                st.stop()
        return data
...
if data is None or data.empty:
    if st.session_state.data is not None and len(st.session_state.data) > 0:
        ...
        st.warning("Esperando primeros datos...")
        st.stop()
...
if enable_alerts:
    if rsi_last > 70:
        st.warning("⚠️ **Alerta RSI**: Sobrecompra (RSI > 70)")
        st.toast("⚠️ Sobrecompra detectada!", icon="⚠️")
    elif rsi_last < 30:
        st.success("✅ **Alerta RSI**: Sobreventa (RSI < 30)")
        st.balloons()
...
# --- Predicciones corto plazo ---
if len(data) >= 80:
    with st.expander("🤖 Predicciones y Comparación de Precisión", expanded=False):
        ...
        if len(X) > 0:
            ...
            if predictions:
                st.subheader("📊 Predicciones de Múltiples Modelos")
...
with st.expander("📈 Predicción a Futuro (3 días de historial)", expanded=False):
    try:
        ...
        st.warning(f"⚠️ Prophet no disponible: {e}")
        model_available = False

    if model_available:
        try:
            with st.spinner("⏳ Cargando historial para predicción..."):
                data_long = yf.download(ticker, period="30d", interval="1h")
                if data_long.empty or len(data_long) < 20:
                    st.warning("⚠️ No hay suficientes datos históricos para predecir.")
                else:
                    ...
                    future_periods = {"6 horas": 6, "1 día": 24, "3 días": 72}
        ...
        st.info("ℹ️ Modelo Prophet no disponible.")
                ...
                if all_articles.get('status') == 'ok' and len(all_articles['articles']) > 0:
                    for article in all_articles['articles']:
                        ...
                        <div class="news-card">
                            ...
                            <a href="{article['url']}" target="_blank" style="color:#0ff;">Leer más</a>
                        </div>
                        """, unsafe_allow_html=True)
                else:
...
# --- Gráfico interactivo ---
st.subheader("📊 Gráfico Interactivo (Zoom + Hover + Predicción)")
...
fig.add_trace(go.Bar(x=data.index, y=data['Volume'], name='Volumen', marker_color='green'), row=2, col=1)
fig.add_trace(go.Scatter(x=data.index, y=data['RSI'], name='RSI', line=dict(color='cyan')), row=3, col=1)
fig.add_hline(y=70, line_dash="dash", line_color="red", row=3, col=1)
fig.add_hline(y=30, line_dash="dash", line_color="green", row=3, col=1)

fig.update_layout(
    title_text=f"{ticker} — Seguimiento en Tiempo Real",
